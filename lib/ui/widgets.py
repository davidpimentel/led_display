...
class Text(Widget):
    def __init__(self, text: str, font: str = "5x8", color: Color = None):
        ...
        self.width = FontRegistry.text_width(self.font_name, self.text)
        self.color = color

    def measure(self, available_width, available_height):
        return Size(self.width, self.font.height)

    def paint(self, canvas, x, y, available_width, available_height):
        graphics.DrawText(
            canvas, self.font, x, y + self.font.baseline, self.color.graphics, self.text
        )


class AnimatedText(Text):
    def __init__(
        self, text: str, font: str = "5x8", color: Color = None, animator=None
    ):
    ...
    def paint(self, canvas, x, y, available_width, available_height):
        ...
        super().paint(
            canvas, x + offset.x, y + offset.y, available_width, available_height
        )


class Rect(Widget):
    def __init__(
        self, width: int, height: int, color: Color = None, filled: bool = True
    ):
        self.w = width
        self.h = height
        self.color = color
    ...
    def paint(self, canvas, x, y, available_width, available_height):
        ...
        if self.filled:
            ...
            graphics.DrawLine(
                canvas, x, y + self.h - 1, x + self.w - 1, y + self.h - 1, gc
            )
            graphics.DrawLine(canvas, x, y, x, y + self.h - 1, gc)
            graphics.DrawLine(
                canvas, x + self.w - 1, y, x + self.w - 1, y + self.h - 1, gc
            )
